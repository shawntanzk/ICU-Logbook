...
function CaseItem({
  item, onPress, directory, currentUserId,
}: {
  item: CaseLog;
  onPress: () => void;
  ...
  // Surface the relationship between the viewer and this case.
  // "yours" if you authored it, otherwise show the owner's name plus
  // a tag for whether you're its supervisor or observer.
  ...
  const relationTag = isMine ? null
    : youSupervised ? 'You supervised'
    : youObserved ? 'You observed'
    : null;
        ...
        <View style={styles.itemMetaRow}>
          ...
          {relationTag && (
            <View style={styles.relationTag}>
              <Text style={styles.relationTagText}>{relationTag}</Text>
            </View>
          )}
        </View>
...
const styles = StyleSheet.create({
  ...
  itemMetaRow: { flexDirection: 'row', alignItems: 'center', gap: 4, marginBottom: 4 },
  itemOwner: { fontSize: FONT_SIZE.xs, color: COLORS.textMuted, fontWeight: '500' },
  relationTag: {
  ...
  relationTagText: { fontSize: 10, fontWeight: '700', color: COLORS.accent },
